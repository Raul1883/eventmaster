...
app.post("/api/login", (req, res) => {
  ...
  db.get(
    ...
    `,
    [loginOrEmail, loginOrEmail, password],
    (err, row) => {
      if (err) {
        res.status(500).json({ error: "Ошибка при проверке пользователя" });
      } else if (row) {
  ...
  db.get(
    ...
    (err, row) => {
      if (err) {
        ...
        res
          .status(404)
          .json({ error: "Пользователь с таким логином не найден" });
      }
    }
  );
});
...
app.post("/api/events/:id/:action", isAuthenticated, (req, res) => {
  ...
  db.get(
    `SELECT participants, user_id, title FROM Events WHERE id = ?`,
    [eventId],
    (err, row) => {
      if (err) {
      ...
      try {
        ...
        if (!Array.isArray(participants)) {
          console.warn(
            `Invalid participants format for event ${eventId}:`,
            row.participants
          );
          participants = [];
        }
        ...
        console.error(
          `Parsing error for participants in event ${eventId}:`,
          e.message
        );
        participants = [];
      }
      ...
      if (action === "join") {
        if (participants.includes(login)) {
          ...
          res
            .status(400)
            .json({ error: "Вы уже участвуете в этом мероприятию" });
          return;
        }
        participants.push(login);
      } else if (action === "leave") {
        const index = participants.indexOf(login);
        console.log("Checking if user is in participants:", {
          login,
          index,
          participants,
        });
        if (index === -1) {
          ...
          res
            .status(400)
            .json({ error: "Вы не участвуете в этом мероприятию" });
          return;
        }
  ...
  db.get(
    ...
    (err, row) => {
      ...
      if (row) {
        res
          .status(400)
          .json({ error: "Эта почта уже используется другим пользователем" });
        return;
      }
...
app.post("/api/events", isAuthenticated, (req, res) => {
  ...
  const participantsArray = Array.isArray(participants)
    ? participants.filter((p) => p && typeof p === "string" && p !== creator)
    : [];
  ...
  console.log(
    "Participants array after processing (excluding creator):",
    participantsArray
  );
  ...
  db.run(
    ...
    function (err) {
      ...
      if (participantsArray.length === 0) {
        console.log(
          "No valid participants provided, skipping notification creation."
        );
          ...
          db.get(
            ...
            (err, user) => {
              if (err) {
                ...
                return reject(
                  new Error(`Database error for user ${login}: ${err.message}`)
                );
              }
              if (!user) {
                console.warn(
        ...
        .then((results) => {
          ...
          if (errors.length > 0) {
            ...
            return res.status(500).json({
              success: false,
              error:
                "Мероприятие создано, но уведомления не отправлены для некоторых участников",
              eventId,
              details: errors,
            });
          }
  ...
  db.get(
    ...
    (err, row) => {
      ...
      if (!row) {
        ...
        res
          .status(403)
          .json({ error: "Только создатель мероприятия может его удалить" });
        return;
      }
      ...
      const events = rows.map((row) => {
        ...
        try {
          ...
          if (!Array.isArray(parsedParticipants)) {
            console.warn(
              `Некорректный формат participants для события ${row.id}:`,
              row.participants
            );
            parsedParticipants = [];
          }
          ...
          console.error(
            `Ошибка парсинга participants для события ${row.id}:`,
            e.message
          );
          parsedParticipants = [];
        }
        ...
        try {
          ...
          console.error(
            `Ошибка парсинга route_data для события ${row.id}:`,
            e.message
          );
